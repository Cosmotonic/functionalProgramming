(* Example solutions to exam BFNP-F2015, june 2015 *)
...
let student = Object [("Name", String "Per Simonsen");
                      ("Field", String "BSWU");
                      ("Course", Record (Object [("BFNP", String "10");
                                                 ("BPRD", String "7")]))]
...
let ppJSONlite json =
  ...
  and ppJSONlite' indent = function
    Object xs ->
      let ppkv (k,v) = nl + (space (indent+2)) + (ppQuote k) + " : " + (ppValue' (indent+2) v)
      "{" + (String.concat "," (List.map ppkv xs)) + nl + (space indent) + "}"
  ppJSONlite' 0 json
...
let expandRef json =
  ...
  let rec expandRefValue = function
      String s -> String s
  ...
  and expandRefJSONlite = function
    Object xs ->
      Object (List.map (fun (k,v) -> (k,expandRefValue v)) xs)
